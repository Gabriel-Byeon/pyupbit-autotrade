...
def search_onetime(settingRSI):
    ...
    if valid_symbols:
        valid_symbols_message = "다음 심볼들은 거래가 가능합니다:\n" + "\n".join(valid_symbols)
        send_discord_message(valid_symbols_message)
    else:
        send_discord_message("거래 가능한 심볼을 찾을 수 없습니다.")
...
while True:
    try:
        valid_symbols = search_onetime(25)
        for symbol in valid_symbols:
            avg_price, volume = buyRSI(symbol, 30, 10)  # 여기서 10은 레버리지 배수입니다.
